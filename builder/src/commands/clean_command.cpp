...
namespace crosside::commands
{
    namespace
    {
        ...
        {
            int removed = 0;
            fs::path runtimeRoot = repoRoot / "Templates" / target / "runtime";
            if (target == "desktop")
            {
                runtimeRoot = repoRoot / "Templates" / "Desktop" / "runtime";
            }
            else if (target == "web")
            {
                runtimeRoot = repoRoot / "Templates" / "Web" / "runtime";
            }
            else if (target == "android")
            {
                runtimeRoot = repoRoot / "Templates" / "Android" / "runtime";
            }

            removed += crosside::io::removePath(runtimeRoot, dryRun, ctx) ? 1 : 0;
            removed += crosside::io::removePath(repoRoot / "bin" / target, dryRun, ctx) ? 1 : 0;
            if (target == "desktop" || target == "web" || target == "android")
            {
                removed += crosside::io::removePath(repoRoot / "build" / "builder" / "_empty_content", dryRun, ctx) ? 1 : 0;
            }
            return removed;
        }
    ...
    int runCleanCommand(const crosside::Context &ctx, const fs::path &repoRoot, const std::vector<std::string> &args)
    {
        ...
        if (opt.kind == "runner")
        {
            ...
            {
                ctx.log("Clean runner template -> ", target);
                removed += cleanRunnerTarget(ctx, repoRoot, target, opt.dryRun);
            }
        }
        else
        {
            auto project = crosside::model::loadKnownWorkspaceProject(repoRoot, opt.name, ctx);
            ...
            for (const auto &target : opt.targets)
            {
                ctx.log("Clean project ", project->name, " -> ", target);
