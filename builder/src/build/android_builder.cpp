...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <application
        ...
        std::optional<fs::path> resolveAndroidNativeAppGlueDir(const fs::path &repoRoot, const AndroidToolchain &tc)
        {
            const std::vector<fs::path> candidates = {
                repoRoot / "vendor" / "android",
                tc.androidNdk / "sources" / "android" / "native_app_glue",
            };
            ...
            for (const auto &dir : candidates)
            {
                ...
                const fs::path header = dir / "android_native_app_glue.h";
                if (fs::exists(header, ec) && fs::is_regular_file(header, ec))
                {
                    return dir;
                }
            }
        ...
        std::optional<fs::path> resolveAndroidNativeAppGlueSource(const fs::path &repoRoot, const AndroidToolchain &tc)
        {
            const auto glueDir = resolveAndroidNativeAppGlueDir(repoRoot, tc);
            if (!glueDir.has_value())
            {
                return std::nullopt;
            }

            const fs::path source = glueDir.value() / "android_native_app_glue.c";
            std::error_code ec;
        ...
        std::vector<fs::path> collectProjectSourcesAndroid(
            ...
            const crosside::Context &ctx)
        {
            ...
            const auto nativeAppGlue = resolveAndroidNativeAppGlueSource(repoRoot, tc);
            if (nativeAppGlue.has_value())
            {
                const fs::path full = fs::absolute(nativeAppGlue.value());
                const std::string key = pathString(full);
                if (seen.insert(key).second)
                {
                    out.push_back(full);
        ...
        bool buildProjectForAbi(
            ...
            bool fullBuild)
        {
            const auto sources = collectProjectSourcesAndroid(project, repoRoot, tc, ctx);
            if (sources.empty())
            {
                ...
                if (fs::exists(outLib, ec) && fs::is_regular_file(outLib, ec))
                {
                    ctx.log("Reuse prebuilt Android runner ", abi.name, ": ", outLib.string());
                    return true;
                }

                ctx.error("No compilable Android sources for project ", project.name, " and no prebuilt lib at ", outLib.string());
                return false;
            }
            ...
            const auto nativeAppGlueDir = resolveAndroidNativeAppGlueDir(repoRoot, tc);
            if (nativeAppGlueDir.has_value())
            {
                addIncludeFlag(ccFlags, cppFlags, nativeAppGlueDir.value());
            }
