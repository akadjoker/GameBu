...
DIV_FONT *DIV_LoadFont(const char *filename)
{
    ...
    // Read charset
    if (fread(&font->charset, sizeof(int32_t), 1, fp) != 1)
    {
        free(font);
        fclose(fp);
        TraceLog(LOG_ERROR, "DIV_LoadFont: Cannot read charset");
        return NULL;
    }
    ...
    // Read character data
    if (fread(font->chardata, sizeof(DIV_CHARDATA), 256, fp) != 256)
    {
        if (font->has_palette)
        {
            // Already read palette, might not be critical
            TraceLog(LOG_WARNING, "DIV_LoadFont: Incomplete character data");
        }
        else
        {
            free(font);
            fclose(fp);
            TraceLog(LOG_ERROR, "DIV_LoadFont: Cannot read character data");
            return NULL;
        }
    }

    // Arrange byte order and validate all character data
    for (int i = 0; i < 256; i++)
    {
        ARRANGE_DWORD((uint32_t *)&font->chardata[i].width);
        ARRANGE_DWORD((uint32_t *)&font->chardata[i].height);
        ARRANGE_DWORD((uint32_t *)&font->chardata[i].xadvance);
        ARRANGE_DWORD((uint32_t *)&font->chardata[i].yadvance);
        ARRANGE_DWORD((uint32_t *)&font->chardata[i].xoffset);
        ARRANGE_DWORD((uint32_t *)&font->chardata[i].yoffset);
        ARRANGE_DWORD((uint32_t *)&font->chardata[i].fileoffset);

        // Validate dimensions (sanity check)
        if (font->chardata[i].width > 0 && font->chardata[i].height > 0)
        {
            if (font->chardata[i].width > 1024 || font->chardata[i].height > 1024)
            {
                TraceLog(LOG_WARNING, "DIV_LoadFont: Invalid glyph %d dimensions (%dx%d), skipping",
                         i, font->chardata[i].width, font->chardata[i].height);
                font->chardata[i].width = 0;
                font->chardata[i].height = 0;
            }
        }
    }
    ...
    for (int n = 0; n < 256; n++)
    {
        ...
        if (font->chardata[n].width > 1024 || font->chardata[n].height > 1024)
        {
            TraceLog(LOG_WARNING, "DIV_LoadFont: Skipping glyph %d with invalid size (%dx%d)",
                     n, font->chardata[n].width, font->chardata[n].height);
            continue;
        }
        ...
        if (font->chardata[n].fileoffset < 0 || font->chardata[n].fileoffset >= file_size)
        {
            TraceLog(LOG_WARNING, "DIV_LoadFont: Invalid file offset for glyph %d (%d), skipping",
                     n, font->chardata[n].fileoffset);
        ...
        {
            TraceLog(LOG_WARNING, "DIV_LoadFont: Invalid widthb %d for glyph %d, skipping", widthb, n);
            continue;
        }
        ...
        { // 100MB limit
            TraceLog(LOG_WARNING, "DIV_LoadFont: Glyph %d too large (%zu bytes), skipping", n, image_size);
            continue;
        }
        ...
        font->glyphs[n].data = malloc(image_size);

        if (!font->glyphs[n].data)
        {
            TraceLog(LOG_WARNING, "DIV_LoadFont: Failed to allocate memory for glyph %d", n);
            continue;
        }
        ...
        for (int y = 0; y < height; y++)
        {
            ...
            if (!line)
            {
                TraceLog(LOG_ERROR, "DIV_LoadFont: Failed to allocate line buffer for glyph %d", n);
                UnloadImage(font->glyphs[n]);
                font->glyphs[n].data = NULL;
                break;
            }

            size_t bytes_read = fread(line, 1, widthb, fp);
            if (bytes_read != (size_t)widthb)
            {
                TraceLog(LOG_WARNING, "DIV_LoadFont: Incomplete read for glyph %d line %d", n, y);
                free(line);
                UnloadImage(font->glyphs[n]);
                font->glyphs[n].data = NULL;
                break;
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                else if (bpp == 32)
                {
                    ...
                    {
                        int idx = x * 4;
                        dest[(y * width + x) * 4 + 0] = line[idx + 0];
                        dest[(y * width + x) * 4 + 1] = line[idx + 1];
                        dest[(y * width + x) * 4 + 2] = line[idx + 2];
                        dest[(y * width + x) * 4 + 3] = line[idx + 3];
                        continue;
                    }
                    colorIndex = 255;
                }
                else
                {
                    ...
                    int byte_pos = x * bpp / 8;
                    colorIndex = (byte_pos < widthb) ? line[byte_pos] : 0;
                }

                int destPos = (y * width + x) * 4;

                if (bpp == 8 && font->has_palette)
                {
        ...
        if (font->glyphs[n].data)
        {
            ...
            TraceLog(LOG_INFO, "DIV_LoadFont: Loaded glyph %d (%dx%d)", n, width, height);
        }
    }
    ...
    for (int i = 32; i < 127; i++)
    {
        if (font->glyphs[i].data)
        {
            ...
            Rectangle dest = {(float)x, (float)y, (float)font->glyphs[i].width, (float)font->glyphs[i].height};
            ImageDraw(&sheet, font->glyphs[i],
                      (Rectangle){0, 0, (float)font->glyphs[i].width, (float)font->glyphs[i].height},
                      dest, WHITE);
        }
    }

    
    ExportImage(sheet, "font.png");
